...
sub typesSettings(){
    my $self = shift; 
    $self->{_typesSettings} ||= Model::APRTypesSettings->load($self->{id},'idType');
}
sub list_backoffice(){
    my $self = shift;
    my @buffer;
...
use Model;use Core::DB;use Core::Gallery;
use Cfg;
...
sub types(){
	my $self = shift; 
	$self->{_types} ||= Model::APRTypes->list();
}
sub load_type(){
	my $self = shift; 
	$self->{_load_type} ||= Model::APRTypes->load($self->{type});
}
sub image(){
    my $self = shift; 
