...
sub new {
	...
	$this->{page} = shift;
	my $m = Model::Meta->load('aprpage','what');
	if ($m) {
...
sub getTitle {
	...
	unless ($this->{title} ) {
		...
		$this->{title} =~ s/%{title}/$this->{page}->{title}/g;
	}
	return $this->{title};
}
...
sub getDescription {
	...
	unless ($this->{description} ) {
		...
		$this->{description} =~ s/%{description}/$this->{page}->{Description}/g;
	}
...
sub getKeywords {
	...
	unless ($this->{keywords} ) {
		...
		$this->{keywords} =~ s/%{keywords}/$this->{page}->{metakeywords}/g;
	}
