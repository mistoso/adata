...
sub new {
	...
	$this->{type} = shift;
	my $m = Model::Meta->load('aprtype','what');
	if ($m) {
...
sub getTitle {
	...
	unless ($this->{title} ) {
		...
		$this->{title} =~ s/%{title}/$this->{type}->{title}/g;
	}
	return $this->{title};
}
...
sub getDescription {
	...
	unless ($this->{description} ) {
		...
		$this->{description} =~ s/%{description}/$this->{type}->{Description}/g;
	}
...
sub getKeywords {
	...
	unless ($this->{keywords} ) {
		...
		$this->{keywords} =~ s/%{keywords}/$this->{type}->{metakeywords}/g;
	}
