package Core::Meta::PRODUCTACCES;
use Model::Meta;
use Model::Category;
use Core::DB;
...
sub new {
	...
	my $m = Model::Meta->load('productacces','what');
	if ($m) {
...
##########
        ...
        $this->{f_block_left} =~ s/%{price}/$this->{salemod}->{price}/g;
...
##########
        ...
        $this->{s_block} =~ s/%{price}/$this->{salemod}->{price}/g;
...
##########
        ...
        $this->{f_block_right} =~ s/%{price}/$this->{salemod}->{price}/g;
