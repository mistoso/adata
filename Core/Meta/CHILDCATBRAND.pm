...
sub new {
	...
	my $m = Model::Meta->load('child_category_brand','what');
	if ($m) {
...
sub getTitle {
	...
	unless ($this->{title}) {
		...
		my $price_limit = $this->{cat}->price_limit;

		use Core::Price;
		my $prc = Core::Price->new();
		my $prc_min = $prc->getByCode($price_limit->{min_price},"UAH");
		my $prc_max = $prc->getByCode($price_limit->{max_price},"UAH");
...
##########
		...
		$this->{title} =~ s/%{brand}/$this->{brand}->{name}/g;
		...
		if($this->{brand}->{rusName}){
		    $this->{title} =~ s/%{brandrusName}/$this->{brand}->{rusName}/g;
		}else{
		    $this->{title} =~ s/%{brandrusName}/$this->{brand}->{name}/g;
		} 
...
##########
		...
		$this->{description} =~ s/%{brand}/$this->{brand}->{name}/g;
		if($this->{brand}->{rusName}){
		    $this->{description} =~ s/%{brandrusName}/$this->{brand}->{rusName}/g;
		}else{
		    $this->{description} =~ s/%{brandrusName}/$this->{brand}->{name}/g;
		} 
	}
...
##########
		...
		$this->{keywords} =~ s/%{brand}/$this->{brand}->{name}/g;
		$this->{keywords} =~ s/%{brandrusName}/$this->{brand}->{rusName}/g;
		if($this->{brand}->{rusName}){
		    $this->{keywords} =~ s/%{brandrusName}/$this->{brand}->{rusName}/g;
		}else{
		    $this->{keywords} =~ s/%{brandrusName}/$this->{brand}->{name}/g;
		} 
...
##########
		...
		$this->{f_block_left} =~ s/%{brand}/$this->{brand}->{name}/g;
		if($this->{brand}->{rusName}){
		    $this->{f_block_left} =~ s/%{brandrusName}/$this->{brand}->{rusName}/g;
		}else{
		    $this->{f_block_left} =~ s/%{brandrusName}/$this->{brand}->{name}/g;
		} 
...
##########
		...
		$this->{s_block} =~ s/%{brand}/$this->{brand}->{name}/g;
		if($this->{brand}->{rusName}){
		    $this->{s_block} =~ s/%{brandrusName}/$this->{brand}->{rusName}/g;
		}else{
		    $this->{s_block} =~ s/%{brandrusName}/$this->{brand}->{name}/g;
		} 
...
##########
		...
		$this->{f_block_right} =~ s/%{brand}/$this->{brand}->{name}/g;
		if($this->{brand}->{rusName}){
		    $this->{f_block_right} =~ s/%{brandrusName}/$this->{brand}->{rusName}/g;
		}else{
		    $this->{f_block_right} =~ s/%{brandrusName}/$this->{brand}->{name}/g;
		} 
